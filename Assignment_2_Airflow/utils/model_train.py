...
def load_datasets(gold_dir, lag_months=4, min_months_required=9, max_snapshot_date=None):
    ...
    # Check existence of feature and label store (prevents failures on first few months)
    if not os.path.exists(feat_dir) or not os.path.exists(lab_dir):
        logger.warning(f"⚠️ Missing feature_store or label_store in {gold_dir}. Skipping retrain.")
        return None, None
...
def save_model(model, metadata, save_dir):
    # Ensure parent directories exist first (handles missing bind mounts)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_path", default="config/ML_config.yaml", help="Optional config file for model parameters")
